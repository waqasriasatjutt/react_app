...
export async function fetchProductBySlug(slug) {
  ...
  // product.image is the multi-image model when present. Try fetching, fall back to main image only.
  let extraIds = [];
  try {
    const imgs = await searchRead(
      'product.image',
      [['product_tmpl_id', '=', p.id]],
      ['id'],
      { limit: 8, order: 'sequence asc, id asc' },
    );
    extraIds = imgs.map((i) => i.id);
  } catch {
    extraIds = [];
  }
  ...
  const gallery = [
    ...
    ...extraIds.map((gid) => imageUrl('product.image', gid, 'image_1024')),
  ];
